...
def calculate_precision_recall_f1(pred_boxes, true_boxes, iou_threshold=0.5):
    ...
    matched_gt_boxes = set()  # Track matched ground truth boxes
    for pred_box in pred_boxes:
        ious = [calculate_iou(pred_box, tb) for tb in true_boxes]
        max_iou_index = np.argmax(ious)  # Index of GT box with max IoU
        max_iou = ious[max_iou_index]

        if max_iou >= iou_threshold and max_iou_index not in matched_gt_boxes:
            TP += 1
            matched_gt_boxes.add(max_iou_index)
        else:
            FP += 1

    FN -= len(matched_gt_boxes)  # Adjust FN based on matched GT boxes
